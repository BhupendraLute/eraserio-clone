...
export function DiagramCanvas() {
  ...
  const { transform, svgRef, handlers, zoomIn, zoomOut, fitToContent } = usePanZoom();
    ...
    <div className="relative h-full w-full overflow-hidden bg-muted/20">
      ...
      >
        <defs>
          ...
          >
            <path d="M0,0 L10,5 L0,10 z" className="fill-current text-muted-foreground" />
          </marker>
        </defs>

        <g transform={`translate(${transform.x}, ${transform.y}) scale(${transform.scale})`}>
          {edges.map((edge, i) => {
            const isDynamic = Boolean(nodeOverrides[edge.from] || nodeOverrides[edge.to]);
      ...
      <div className="absolute bottom-4 right-4 flex flex-col gap-1 rounded-md border bg-background p-1 shadow-sm">
        ...
        <Button variant="ghost" size="icon" onClick={() => fitToContent(nodes)} aria-label="Fit to content">
...
function NodeView({ node, scale, onDrag, onResetPosition }: NodeViewProps) {
  const dragStart = useRef<{ pointerX: number; pointerY: number; nodeX: number; nodeY: number } | null>(
    null
  );
